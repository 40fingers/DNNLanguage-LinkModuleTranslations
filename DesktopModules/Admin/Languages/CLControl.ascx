        ...
        <asp:Repeater ID="rDnnModules" runat="server"
            ...
            <ItemTemplate>
                <tr class="<%# Container.ItemIndex % 2 == 0 ? "rgAltRow" : "rgRow" %>">
                    <asp:Repeater ID="rDnnModule" runat="server" OnItemCommand="rDnnModule_OnItemCommand">
                        <ItemTemplate>
                            <td class="moduleInfo<%# ((Container.ItemIndex == 0) && (Data.Pages.Count>1)) ? " headcol" : "" %>">
                                <asp:Panel ID="pAddModule" runat="server" Visible='<%# Eval("NotExist") %>' class="dnnFormItem">
                                    ...
                                    <div id="UnlinkedModuleActions" class="infoActions" runat="server" visible='<%# (Container.ItemIndex > 0) %>'>
                                        <asp:HiddenField runat="server" id="hfPasteDefaultLanguageGuid" Value='<%# Eval("PasteDefaultLanguageGuid") %>' />
                                        <span class="pasteButton" data-pastebutton='<%# Eval("ModuleDefID") + "-" + Eval("CultureCode") %>' style="display: none;">
                                            <asp:HiddenField runat="server" id="hfPasteTabModuleId" />
                                            <asp:LinkButton runat="server"
                                                            CommandName="LinkTabModule"
                                                            CausesValidation="False"
                                                            ToolTip='<%# LocalizeString("UnlinkedModule.Paste")%>'>
                                                <dnn:DnnImage ID="DnnImage3" runat="server" ResourceKey="UnlinkedModule.Paste" IconKey="FileDownload" IconStyle="Gray" />
                                            </asp:LinkButton>
                                        </span>
                                    </div>
                                </asp:Panel>
                                <asp:Panel ID="pDnnModule" runat="server" Visible='<%# Convert.ToBoolean(Eval("Exist")) && Convert.ToBoolean(Eval("CanViewModule")) %>' class="dnnFormItem">
                                    ...
                                    <div class="infoCheckBoxes unlinkedModuleActions" runat="server" visible='<%# ( Container.ItemIndex > 0 && Convert.ToBoolean(Eval("LocalizedVisible")) == false) %>'>
                                        <span id="cutModule" runat="server">
                                            <a href="#" title='<%# LocalizeString("UnlinkedModule.Cut")%>' onclick='cutUnlinkedModule(<%# Eval("TabModuleID") %>, "<%# Eval("ModuleDefID") + "-" + Eval("CultureCode") %>"); return false;'  data-moduleid='<%# ModuleInfo(Eval("ModuleID")).ModuleID %>'><dnn:DnnImage ID="imgCutModule" runat="server" ResourceKey="UnlinkedModule.Cut" IconKey="FileMove" IconStyle="Gray" /></a>
                                        </span>
                                    </div>
...
<script type="text/javascript">
    ...
    function cutUnlinkedModule(pasteTabModuleId, pasteButtonData) {
        // hide all pasteButtons
        $(".pasteButton").hide();
        // empty paste-fields
        $(".pasteButton input[type='hidden']").val('');
        // show the possible ones
        $(".pasteButton[data-pastebutton='" + pasteButtonData + "']").show();
        $(".pasteButton input[type='hidden']").val(pasteTabModuleId);
        // save the data needed for the relink --> tabmoduleId
        // show the possible paste buttons
        // -- same language
        // -- has Copy Module option now visible
        // -- same ModuleDefID and not present in current language
    }
</script>
